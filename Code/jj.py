...
class HealthBar:
    def __init__(self, max_hp, scale_factor=0.5):
        self.max_hp = max_hp
        self.remaining_hp = max_hp
        self.scale_factor = scale_factor

        # Load health bar images
        self.Hp_bar = pygame.image.load('D:/SpiritKnight/Sprites/HP1.png')
        self.Hp_bar = pygame.transform.scale(self.Hp_bar, (int(self.Hp_bar.get_width() * scale_factor), int(self.Hp_bar.get_height() * scale_factor)))
        self.Hp_bar_rect = self.Hp_bar.get_rect(topleft=(0, 0))

        self.Hp_2 = pygame.image.load('D:/SpiritKnight/Sprites/HP2.png')
        self.Hp_2 = pygame.transform.scale(self.Hp_2, (int(self.Hp_2.get_width() * scale_factor), int(self.Hp_2.get_height() * scale_factor)))
        self.Hp_2_rect = self.Hp_2.get_rect(topleft=(0, 0))

    def update_health(self, amount):
        self.remaining_hp = max(0, min(self.max_hp, self.remaining_hp + amount))
    ...
    def draw(self, screen):
        ...
        remaining_width = int((self.remaining_hp / self.max_hp) * self.Hp_bar.get_width())

        # Create a surface for the remaining health
        remaining_health_surface = pygame.Surface((remaining_width, self.Hp_bar.get_height()))
        remaining_health_surface.fill((255, 0, 0))  # Red color

        # Blit the remaining health bar onto the screen
        screen.blit(remaining_health_surface, self.Hp_bar_rect.topleft)

        # Draw health text
        font = pygame.font.Font('D:/SpiritKnight/Font/properhitboxglobal.ttf', 18)
        hp_text_white = font.render(f"{self.remaining_hp}/{self.max_hp}", True, (255, 255, 255))
        hp_text_rect = hp_text_white.get_rect(center=(self.Hp_bar_rect.centerx, self.Hp_bar_rect.centery))
...
# Potion
potion_gif_path = 'D:/SpiritKnight/Sprites/potion.gif'
...
try:
    while True:
        ...
        potion_frame = potion_frame.resize((100,100), Image.Resampling.LANCZOS)
...
potion_rect = potion_frames[0].get_rect(center = (width//2, height//2 + 200))

# New Skeleton
new_skeleton_gif_path = 'D:/SpiritKnight/Sprites/Skele.gif'
new_skeleton_gif = Image.open(new_skeleton_gif_path)
new_skeleton_frames = []
try:
    while True:
        new_skeleton_frame = new_skeleton_gif.copy()
        new_skeleton_frame = new_skeleton_frame.convert("RGBA")
        new_skeleton_frames.append(pygame.image.fromstring(new_skeleton_frame.tobytes(), new_skeleton_frame.size, new_skeleton_frame.mode))
        new_skeleton_gif.seek(len(new_skeleton_frames))  # Move to the next frame
except EOFError:
    pass

new_skeleton_rect = new_skeleton_frames[0].get_rect(center = (width//2 + 200, height//2 - 200))
...
new_skeleton_health = 40
new_skeleton_killed = False
new_skeleton_frame_counter = 0
new_skeleton_frame_index = 0
...
while True:
    ...
    # Update new skeleton frame
    new_skeleton_frame_counter += 1
    if new_skeleton_frame_counter >= frame_update_rate:
        new_skeleton_frame_counter = 0
        new_skeleton_frame_index = (new_skeleton_frame_index + 1) % len(new_skeleton_frames)

    # Draw health bar
    health_bar.draw(screen)

    # Display new skeleton
    screen.blit(new_skeleton_frames[new_skeleton_frame_index], new_skeleton_rect)

    # Check collision with new skeleton
    if char_rect.colliderect(new_skeleton_rect) and current_time - hit_timer > hit_delay:
        new_skeleton_health -= damage
        hit_timer = current_time
        if new_skeleton_health <= 0:
            new_skeleton_rect.topleft = (-1000,-1000)
            new_skeleton_killed = True

    # Display potion if new skeleton is killed
    if new_skeleton_killed:
