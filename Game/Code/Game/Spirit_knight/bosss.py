def spawn_boss():
    ...
    Music_folder = os.path.join(script_dir, 'MusicGame')
    Font_folder = os.path.join(script_dir, 'FontGame')
    ...
    boss_jump2_frame_counter = 0
    boss_jump2_frame_index = 0
    ...
    # Slash PNGs
    slash_right = pygame.image.load(os.path.join(Sprites_folder,'wind burst.png'))
    slash_left = pygame.image.load(os.path.join(Sprites_folder,'wind burst2.png'))
    ...
    while True:
        ...
        if boss_attacking:
            ...
            if boss_remaining_hp > 250:
                ...
                boss_jump2_frame_counter += 1
                if boss_jump2_frame_counter >= boss_jump_frame_rate: 
                    boss_jump2_frame_counter = 0
                    boss_jump2_frame_index = (boss_jump2_frame_index + 1) % len(weak_attack_frames)

                # Vẽ hoạt ảnh tấn công yếu
                screen.blit(weak_attack_frames[boss_jump2_frame_index], boss_rect)    
                ...
                if boss_jump2_frame_index == len(weak_attack_frames) - 1:
                    boss_attacking = False
                    boss_jump_timer = current_time  # Cập nhật thời gian tấn công tiếp theo
            ...
            if current_time - boss_jump_timer > boss_attack_interval:
                ...
                if boss_remaining_hp <= 250:
                    ...
                    dmg = 10
                    boss_damage = 20 
                    knife = pygame.image.load(os.path.join(Sprites_folder, 'Teeth.png')).convert_alpha()
                    knife = pygame.transform.scale(knife, (int(knife.get_width()*0.5), int(knife.get_height()*0.5)))
        ...
        if attacking:
            ...
            if slash_rect.colliderect(boss_rect):
                boss_remaining_hp -= slash_dmg
        ...
        # Hiển thị khung hình hoạt ảnh nhận sát thương
            ...
            if flipped: 
                screen.blit(flipped_run_frames[run_frame_index], char_rect)
            else: 
                screen.blit(run_frames[run_frame_index], char_rect)
            run_frame_counter += 1 
